...
class ProcessPipes
{
	...
	public function __construct($useFiles, $ttyMode, $ptyMode = false, $disableOutput = false)
	{
		...
		if ($this->useFiles && !$this->disableOutput) {
			$this->files = array(Process::STDOUT => tempnam(sys_get_temp_dir(), 'sf_proc_stdout'), Process::STDERR => tempnam(sys_get_temp_dir(), 'sf_proc_stderr'),);
			...
			$this->readBytes = array(Process::STDOUT => 0, Process::STDERR => 0,);
		}
	}
	...
	public function getDescriptors()
	{
		...
		return array(array('pipe', 'r'), // stdin
			array('pipe', 'w'), // stdout
			array('pipe', 'w'), // stderr
		);
	}
