...
class Utf8
{
	protected static
		...
		$commonCaseFold = array(array('µ', 'ſ', "\xCD\x85", 'ς', "\xCF\x90", "\xCF\x91", "\xCF\x95", "\xCF\x96", "\xCF\xB0", "\xCF\xB1", "\xCF\xB5", "\xE1\xBA\x9B", "\xE1\xBE\xBE"), array('μ', 's', 'ι', 'σ', 'β', 'θ', 'φ', 'π', 'κ', 'ρ', 'ε', "\xE1\xB9\xA1", 'ι')), $cp1252 = array('', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''), $utf8 = array('€', '‚', 'ƒ', '„', '…', '†', '‡', 'ˆ', '‰', 'Š', '‹', 'Œ', 'Ž', '‘', '’', '“', '”', '•', '–', '—', '˜', '™', 'š', '›', 'œ', 'ž', 'Ÿ');
			...
			case 'string':
				...
				if (preg_match('/[\x80-\xFF]/', $var)) {
					if (n::isNormalized($var, $normalization_form))
						...
						if (isset($n[0]))
							$var = $n; else $var = static::utf8_encode($var);
					}
	...
	{
		if (4 > func_num_args())
			$var = filter_input($type, $var, $filter); else $var = filter_input($type, $var, $filter, $option);
	...
	{
		if (2 > func_num_args())
			$a = filter_input_array($type); else $a = filter_input_array($type, $def, $add_empty);
	...
	static function strpbrk($s, $charlist)
	{
		if (preg_match('/' . self::rxClass($charlist) . '/us', $s, $m))
			return substr($s, strpos($s, $m[0])); else return false;
	}
	...
	static function strtr($s, $from, $to = INF)
	{
		if (INF !== $to) {
			...
			if ($a > $b)
				$from = array_slice($from, 0, $b); else if ($a < $b)
	...
	static function utf8_encode($s)
	{
		...
		if (false === strpos($s, "\xC2"))
			return $s; else return str_replace(self::$cp1252, self::$utf8, $s);
	}
	...
	protected static function rxClass($s, $class = '')
	{
		...
		foreach (self::str_split($s) as $s) {
			if ('-' === $s)
				$class[0] = '-' . $class[0]; else if (!isset($s[2]))
				$class[0] .= preg_quote($s, '/'); else if (1 === iconv_strlen($s, 'UTF-8'))
				$class[0] .= $s; else $class[] = $s;
		}

		$class[0] = '[' . $class[0] . ']';

		if (1 === count($class))
			return $class[0]; else return '(?:' . implode('|', $class) . ')';
	}
	...
	protected static function getData($file)
	{
		...
		if (file_exists($file))
			return unserialize(file_get_contents($file)); else return false;
	}
}
