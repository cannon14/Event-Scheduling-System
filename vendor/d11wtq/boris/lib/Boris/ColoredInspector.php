...
class ColoredInspector implements Inspector
{
	...
	{
		$tests = array('is_null' => '_dumpNull', 'is_string' => '_dumpString', 'is_bool' => '_dumpBoolean', 'is_integer' => '_dumpInteger', 'is_float' => '_dumpFloat', 'is_array' => '_dumpArray', 'is_object' => '_dumpObject');
	...
	public function _buildAst($type, $value, $seen = array())
	{
		...
		return array('name' => $this->_colorize('keyword', $type), 'children' => empty($vars) ? array() : array_combine(array_map(array($this, '_dump'), array_keys($vars)), array_map(function ($v) use ($self, $nextSeen) {
				if (is_object($v)) {
	...
	public function _astToString($node, $indent = 0)
	{
		...
		return implode("\n", array(sprintf('%s(', $node['name']), implode(",\n", array_map(function ($k) use ($self, $children, $indent) {
				...
				}, array_keys($children))), sprintf('%s)', str_repeat(' ', $indent * 2))));
	}

	private function _defaultColorMap()
	{
		return array('integer' => 'light_green', 'float' => 'light_yellow', 'string' => 'light_red', 'bool' => 'light_purple', 'keyword' => 'light_cyan', 'comment' => 'dark_grey', 'default' => 'none');
	}
