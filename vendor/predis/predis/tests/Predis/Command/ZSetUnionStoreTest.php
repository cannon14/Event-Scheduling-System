...
class ZSetUnionStoreTest extends PredisCommandTestCase
{
	...
	{
		$modifiers = array('aggregate' => 'sum', 'weights' => array(10, 100),);
		$arguments = array('zset:destination', 2, 'zset1', 'zset2', $modifiers);

		$expected = array('zset:destination', 2, 'zset1', 'zset2', 'WEIGHTS', 10, 100, 'AGGREGATE', 'sum');
	...
	public function testPrefixKeys()
	{
		...
		$expected = array('prefix:zset:destination', 2, 'prefix:zset1', 'prefix:zset2', 'WEIGHTS', 10, 100, 'AGGREGATE', 'sum');
	...
	public function testStoresUnionInNewSortedSet()
	{
		...
		$this->assertSame(array(array('a', '1'), array('b', '3'), array('d', '3'), array('c', '5')), $redis->zrange('letters:out', 0, -1, 'withscores'));
	...
	public function testStoresUnionWithAggregateModifier()
	{
		...
		$this->assertSame(array(array('a', '1'), array('b', '1'), array('c', '2'), array('d', '3')), $redis->zrange('letters:min', 0, -1, 'withscores'));

		$options = array('aggregate' => 'max');
		$this->assertSame(4, $redis->zunionstore('letters:max', 2, 'letters:1st', 'letters:2nd', $options));
		$this->assertSame(array(array('a', '1'), array('b', '2'), array('c', '3'), array('d', '3')), $redis->zrange('letters:max', 0, -1, 'withscores'));

		$options = array('aggregate' => 'sum');
		$this->assertSame(4, $redis->zunionstore('letters:sum', 2, 'letters:1st', 'letters:2nd', $options));
		$this->assertSame(array(array('a', '1'), array('b', '3'), array('d', '3'), array('c', '5')), $redis->zrange('letters:sum', 0, -1, 'withscores'));
	}
	...
	public function testStoresUnionWithWeightsModifier()
	{
		...
		$this->assertSame(array(array('a', '2'), array('b', '7'), array('d', '9'), array('c', '12')), $redis->zrange('letters:out', 0, -1, 'withscores'));
	}
	...
	public function testStoresUnionWithCombinedModifiers()
	{
		...
		$this->assertSame(4, $redis->zunionstore('letters:out', 2, 'letters:1st', 'letters:2nd', $options));
		$this->assertSame(array(array('a', '10'), array('b', '20'), array('c', '30'), array('d', '45')), $redis->zrange('letters:out', 0, -1, 'withscores'));
	}
