...
class Framework_MockObject_GeneratorTest extends PHPUnit_Framework_TestCase
{
	...
	{
		$mock = $this->generator->getMockForAbstractClass('AbstractMockTestClass', array(), '', true, true, true, array('nonexistentMethod'));
	...
	public function testGetMockForAbstractClassShouldCreateStubsOnlyForAbstractMethodWhenNoMethodsWereInformed()
	{
		...
		$mock->expects($this->any())->method('doSomething')->willReturn('testing');
	...
	{
		return array('className not a string' => array(array(), ''), 'mockClassName not a string' => array('Countable', new StdClass),);
	}
	...
	{
		$mock = $this->generator->getMockForTrait('AbstractTrait', array(), '', true, true, true, array('nonexistentMethod'));
